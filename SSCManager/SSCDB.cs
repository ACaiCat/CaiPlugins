...
namespace SSCManager
{
    ...
    public static class SSCDB
    {
        ...
        public static void RestoryBackBag(this TSPlayer plr)
        {
            ...
            plr.PlayerData = SSCSaver.playerDatas[plr.Index];
            plr.PlayerData.RestoreCharacter(plr);
            SSCSaver.playerDatas[plr.Index] = null;
        }
        public static PlayerData GetPlayerData(int sscid)
        {
            PlayerData playerData = new PlayerData(new TSPlayer(-1));

            try
            {
                using (var reader = TShock.DB.QueryReader("SELECT * FROM SavedCharacter WHERE ID=@0", sscid))
                {
                    if (reader.Read())
                    {
        ...
        public static bool DeletePlayerData(int sscId)
        {
            try
            {
                TShock.DB.Query(
                    "DELETE FROM SavedCharacter WHERE ID = @0;",
                    sscId);
                return true;
            }
            catch (Exception ex)
        ...
        public static List<int> GetAllSSCId()
        {
            List<int> ids = new List<int>();
            try
            {
                using (var reader = TShock.DB.QueryReader("SELECT * FROM SavedCharacter"))
                {
                    while (reader.Read())
                    {
        ...
        public static bool InsertPlayerData(TSPlayer player,int sscId)
        {
